...
############################################################
def get_transform_function(processor, root, fusion, train_with_plv):
    ...
    def transforms(example_batch):
        ...
        for img_path1, img_path2 in zip(example_batch["image1"], example_batch["image2"]):
            ...
            image2 = image2.resize((image1.width, image1.height))
            plvimg = concate_plvimg(img_path1)
...
if __name__ == "__main__":
    ...
    for fusion in FUSION_LIST:
        ...
        print(f"========== Training with ROOT: {root} {fusion}, plv {train_with_plv} ==========")
        ...
        # Set transforms
        train_ds.set_transform(get_transform_function(processor, root, fusion, train_with_plv))
        test_ds.set_transform(get_transform_function(processor, root, fusion, train_with_plv))

        # 5) Define training arguments
        training_args = TrainingArguments(
            output_dir=f"{root}/git_checkpoint_{fusion}",
            learning_rate=5e-5,
            num_train_epochs=10,
            fp16=True,
        ...
        # 7) Train
        trainer.train()
        print(f"========== Finished Training with ROOT: {root} {fusion}, plv {train_with_plv} ==========\n")
