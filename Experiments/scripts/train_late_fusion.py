"""
Training script for Late Fusion Model (Image + EEG)
"""
...
def train_epoch(
    ...
    for batch in pbar:
        ...
        loss = outputs['loss']
        loss_fused = outputs['loss_fused']
        loss_img = outputs['loss_img']
        loss_eeg = outputs['loss_eeg']
    ...
    return {
        ...
        'train/loss_fused': total_loss_fused / num_batches,
        'train/loss_img': total_loss_img / num_batches,
        'train/loss_eeg': total_loss_eeg / num_batches
    }
...
def evaluate(
    ...
    with torch.no_grad():
        for batch in pbar:
            ...
            logits_img = outputs['logits_img']
            logits_eeg = outputs['logits_eeg']
            ...
            preds_eeg = torch.argmax(logits_eeg, dim=-1)

            all_preds.extend(preds.cpu().numpy())
            all_preds_img.extend(preds_img.cpu().numpy())
            all_preds_eeg.extend(preds_eeg.cpu().numpy())
            all_labels.extend(labels.cpu().numpy())

    # Compute metrics
    all_preds = np.array(all_preds)
    all_preds_img = np.array(all_preds_img)
    all_preds_eeg = np.array(all_preds_eeg)
    ...
    acc_img = accuracy_score(all_labels, all_preds_img)
    acc_eeg = accuracy_score(all_labels, all_preds_eeg)
    ...
    metrics = {
        ...
        'eval/recall': recall,
        'eval/f1': f1,
        'eval/accuracy_img': acc_img,
        'eval/accuracy_eeg': acc_eeg
    }
...
def main(args):
    ...
    model = LateFusionModel(
        ...
        image_weight=config['model']['image_weight'],
        eeg_weight=config['model']['eeg_weight']
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--config',
        type=str,
        default='Experiments/configs/late_fusion.yaml',
