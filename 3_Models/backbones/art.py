...
# Try relative import first, fall back to finding hf_config in sys.modules
try:
    from .hf_config import ArtifactRemovalTransformerConfig
except ImportError:
    import sys
    if 'hf_config' in sys.modules:
        ArtifactRemovalTransformerConfig = sys.modules['hf_config'].ArtifactRemovalTransformerConfig
    else:
        # If hf_config is not available, define a minimal version
        from transformers import PretrainedConfig
        class ArtifactRemovalTransformerConfig(PretrainedConfig):
            model_type = "artifact_removal_transformer"
            def __init__(self, in_channels=32, out_channels=32, embedding_size=128,
                         num_encoder_layers=6, num_decoder_layers=6, num_heads=8,
                         feedforward_size=2048, dropout=0.1, max_len=2048,
                         pos_mode='sinusoidal', recon_log_softmax=False,
                         recon_zscore=None, loss_zscore=False, **kwargs):
